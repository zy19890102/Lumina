...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        label.frame = CGRect(x: 0, y: 80, width: UIScreen.main.bounds.width, height: 50)
...
extension ViewController: LuminaDelegate {
    ...
    func streamed(videoFrame: UIImage, from controller: Lumina.LuminaViewController, brightnessValue: CGFloat) {
        print("@@@@@@@@@@@@@---\(brightnessValue)")
        if label.superview == nil {
            controller.view.addSubview(label)
        }
        label.text = "\(brightnessValue)"
    }
